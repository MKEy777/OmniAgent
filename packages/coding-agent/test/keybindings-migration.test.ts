...
describe("keybindings migration", () => {
	...
	it("uses startup shortcut defaults for agents and recent models", () => {
		const keybindings = new KeybindingsManager();
		const effective = keybindings.getEffectiveConfig();

		expect(effective["app.agent.cycle"]).toBe("tab");
		expect(effective["app.model.cycleForward"]).toBe("ctrl+m");
		expect(effective["app.model.cycleBackward"]).toBe("shift+ctrl+m");
	});
});
