...
describe("Coding Agent Tools", () => {
	...
	describe("bash tool", () => {
		...
		it.skipIf(process.platform !== "win32")(
			"should execute explicit cmd commands without resolving bash",
			async () => {
				vi.spyOn(shellModule, "getShellConfig").mockImplementation(() => {
					throw new Error("bash should not be resolved");
				});
				const chunks: Buffer[] = [];
				const ops = createLocalBashOperations();

				const result = await ops.exec("cmd.exe /c echo from-cmd", testDir, {
					onData: (data) => chunks.push(data),
				});

				expect(result.exitCode).toBe(0);
				expect(Buffer.concat(chunks).toString("utf-8")).toContain("from-cmd");
			},
		);
