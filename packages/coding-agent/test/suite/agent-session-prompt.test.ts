...
describe("AgentSession prompt characterization", () => {
	...
	it("expands direct skill aliases before sending the prompt", async () => {
		const tempDir = join(tmpdir(), `pi-skill-alias-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		mkdirSync(tempDir, { recursive: true });
		tempDirs.push(tempDir);
		const skillPath = join(tempDir, "alias-skill.md");
		writeFileSync(skillPath, "# Alias Skill\n\nUse the alias body.");
		...
		await harness.session.prompt("/alias explain this");

		expect(expandedPrompt).toContain('<skill name="alias" location="');
		expect(expandedPrompt).toContain("Use the alias body.");
		expect(expandedPrompt).toContain("explain this");
	});
	...
	it("lets prompt templates take precedence over direct skill aliases", async () => {
		const tempDir = join(tmpdir(), `pi-skill-template-conflict-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		mkdirSync(tempDir, { recursive: true });
		tempDirs.push(tempDir);
		const skillPath = join(tempDir, "review-skill.md");
		writeFileSync(skillPath, "# Review Skill\n\nThis skill should not expand.");
		const template: PromptTemplate = {
			name: "review",
			description: "Review template",
			content: "Template wins: $1",
			filePath: "/virtual/review.md",
			sourceInfo: createSyntheticSourceInfo("/virtual/review.md", {
				source: "local",
				scope: "temporary",
		...
		expect(expandedPrompt).toBe("Template wins: src/index.ts");
	});

	it("does not expand direct skill aliases that conflict with built-in commands", async () => {
		const tempDir = join(tmpdir(), `pi-skill-builtin-conflict-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		mkdirSync(tempDir, { recursive: true });
		tempDirs.push(tempDir);
		const skillPath = join(tempDir, "name-skill.md");
		writeFileSync(skillPath, "# Name Skill\n\nThis skill should not expand.");
		const resourceLoader = {
			...createTestResourceLoader(),
			getSkills: () => ({
				skills: [
		...
		await harness.session.prompt("/name project");

		expect(expandedPrompt).toBe("/name project");
	});
	...
	it("lets extension commands take precedence over direct skill aliases", async () => {
		const tempDir = join(
			tmpdir(),
			`pi-skill-extension-conflict-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		mkdirSync(tempDir, { recursive: true });
		tempDirs.push(tempDir);
		const skillPath = join(tempDir, "testcmd-skill.md");
		writeFileSync(skillPath, "# Test Command Skill\n\nThis skill should not expand.");
		const commandRuns: string[] = [];
