...
describe("AgentSession model and extension characterization", () => {
	...
	it("cycles only through the four most recent session models", async () => {
		const harness = await createHarness({
			models: [
				{ id: "faux-1", name: "One", reasoning: false },
				{ id: "faux-2", name: "Two", reasoning: false },
				{ id: "faux-3", name: "Three", reasoning: false },
				{ id: "faux-4", name: "Four", reasoning: false },
				{ id: "faux-5", name: "Five", reasoning: false },
			],
		});
		harnesses.push(harness);

		for (const id of ["faux-2", "faux-3", "faux-4", "faux-5"]) {
			await harness.session.setModel(harness.getModel(id)!);
		}
		...
		for (const expected of ["faux-3", "faux-4", "faux-5", "faux-2"]) {
			await harness.session.cycleModel();
			expect(harness.session.model?.id).toBe(expected);
		}
	});
