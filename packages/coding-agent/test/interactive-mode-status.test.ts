...
describe("InteractiveMode.cycleAgent", () => {
	test("shows the current coding agent when it is the only agent", () => {
		const fakeThis = {
			showStatus: vi.fn(),
		};

		(InteractiveMode as any).prototype.cycleAgent.call(fakeThis);

		expect(fakeThis.showStatus).toHaveBeenCalledWith("Agent: coding");
	});
});
